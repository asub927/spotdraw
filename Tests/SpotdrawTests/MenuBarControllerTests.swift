...
func testToolMenuContainsTextAndDispatchesSelection() -> PreservationTestResult {
    return runPreservationTest(
        ...
        guard let menu = statusItem?.menu else {
            return (false, "Could not get menu from statusItem")
        }
        guard let toolMenu = menu.items.first(where: { $0.title == "Tool" })?.submenu else {
            return (false, "Could not inspect the MenuBarController Tool submenu")
        }

        // Menu item titles include shortcut suffix, e.g. "Text (T)", so use hasPrefix
