...
internal enum ShortcutScope: Sendable {
...
internal enum ShortcutCategory: String, CaseIterable, Sendable {
    case global = "Global"
...
internal enum ShortcutAction: String, CaseIterable, Sendable {
...
internal final class ShortcutStore: @unchecked Sendable {
    ...
    func resolve(keyCode: UInt16, modifiers: NSEvent.ModifierFlags, scope: ShortcutScope) -> ShortcutAction? {
        ensureIndicesBuilt()
    ...
    func resolveGlobal(keyCode: UInt16, cgFlags: CGEventFlags) -> ShortcutAction? {
        ensureIndicesBuilt()
    ...
    func _loadFromData(_ data: Data?) {
        if let data = data {
            do {
                let decoded = try JSONDecoder().decode([String: StoredBinding].self, from: data)
                overrides = decoded.filter { key, _ in
