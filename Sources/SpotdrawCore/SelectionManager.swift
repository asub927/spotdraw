...
public final class SelectionManager {
    ...
    public func set(_ ids: Set<UUID>) {
        selectedIDs = ids
    }
    ...
    public func boundingBox(in items: [any DrawingItem]) -> CGRect? {
    ...
    /// Requirement 2.7.
    public static func itemsIntersecting(_ marquee: CGRect, in items: [any DrawingItem]) -> Set<UUID> {
        var result = Set<UUID>()
        for item in items {
    ...
    /// Requirement 2.4.
    public static func topmostHit(at point: CGPoint, threshold: CGFloat, in items: [any DrawingItem]) -> (any DrawingItem)? {
