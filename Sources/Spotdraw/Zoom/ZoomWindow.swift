...
// Uses a 30 fps timer to grab a screen capture of the area around the cursor,
// excluding its own window, and renders the zoomed capture inside a circular-masked
// NSImageView with a decorative border ring.
//
// On macOS 14+, uses ScreenCaptureKit (SCScreenshotManager) for capture.
// On macOS 13, falls back to the deprecated CGWindowListCreateImage.
    ...
    private func captureWithScreenCaptureKit() {
        ...
        Task { @MainActor in
            do {
                ...
                // Exclude the zoom bubble from captures
                let zoomWindowID = CGWindowID(window.windowNumber)
                let excludedWindows = content.windows.filter { $0.windowID == zoomWindowID }
                ...
                config.sourceRect = CGRect(
                    ...
                    y: quartzY - captureSize / 2,
                    width: captureSize,
                    height: captureSize
                )
                config.width = Int(bubbleSize)
                config.height = Int(bubbleSize)
                ...
                let nsImage = NSImage(
                    cgImage: cgImage,
                    size: NSSize(width: bubbleSize, height: bubbleSize)
                )
                self.imageView?.image = nsImage
            } catch {
                // Silently drop this frame — next timer tick will retry.
            }
        }
    }

    // MARK: - Legacy Path (macOS 13)
