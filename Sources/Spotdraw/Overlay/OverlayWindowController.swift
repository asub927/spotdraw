...
@MainActor internal final class OverlayWindowController {
    ...
    /// Migrates drawing items whose screen is no longer connected to the primary display.
    /// Called when displays change to prevent items from becoming invisible.
    /// Items are never automatically migrated back when a display reconnects (Requirement 1.5).
    private func migrateOrphanedItems() {
        let activeDisplayIDs = Set(NSScreen.screens.compactMap {
            $0.deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? CGDirectDisplayID
        })
        let primaryID = CGMainDisplayID()

        for item in drawingState.items {
            if !activeDisplayIDs.contains(item.screenID) {
                item.screenID = primaryID
            }
        }
    }
