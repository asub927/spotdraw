...
/// Locates the `NSTextField` that `TextEditingController.begin` installed as a
/// subview of `view`. `begin` adds exactly one such field per call (any previous
/// field is committed and removed before a new one is added), so the first match
/// is unambiguous.
@MainActor
private func installedTextField(in view: OverlayView) -> NSTextField? {
...
func testTextCommitDiscardsWhitespaceOnlyInput() -> PreservationTestResult {
    return runPreservationTest(
        ...
        guard let field = installedTextField(in: view) else {
            return (false, "begin(...) did not install an NSTextField subview on the OverlayView")
        }

        let whitespaceOnly = makeRandomWhitespaceOnlyString(rng: &rng)
        field.stringValue = whitespaceOnly
...
/// where the first is smaller than the second, the bounds measured at the smaller
/// size are no larger in either dimension than those measured at the larger size.
...
func testTextBoundsAreMonotonicInFontSize() -> PreservationTestResult {
    return runPreservationTest(
        "Property 9: Text bounds are monotonic in font size",
        iterations: 150
    ) { rng in
        let string = makeRandomNonEmptyBoundsString(rng: &rng)
        let anchor = CGPoint(
        ...
        guard boundsA.width <= boundsB.width else {
            return (false, "TextAnnotation(string: \(String(reflecting: string)), fontSize: \(fontSizeA)) "
                + "has width \(boundsA.width), which exceeds the width \(boundsB.width) at the larger "
                + "fontSize \(fontSizeB). Width must be monotonic non-decreasing in font size.")
        }
